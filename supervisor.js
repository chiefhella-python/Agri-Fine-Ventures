...
const SupervisorDashboard = {
  ...
  openHarvestModal(ghId) {
    ...
    const priceInput = document.getElementById('supervisor-harvest-price');
  ...
  saveHarvest(e) {
    ...
    const pricePerKg = parseFloat(document.getElementById('supervisor-harvest-price').value) || 0;
    const gh = AFV.greenhouses.find(g => g.id === ghId);
                        ...
                        ${!task.assignedTo ? `
                          <div style="display:flex;gap:4px">
                            <select id="assign-worker-${gh.id}-${task.id}" style="padding:4px;border-radius:4px;border:1px solid var(--blue-pale);font-size:0.7rem;width:80px">
                              <option value="">Select</option>
                              ${workers.map(w => `<option value="${w.id}">${w.name.split(' ')[0]}</option>`).join('')}
                            </select>
