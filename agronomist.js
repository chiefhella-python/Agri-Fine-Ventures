...
const AgronomistDashboard = {
                ...
                return `
                  <div style="margin-bottom:14px;padding:14px;border-radius:var(--radius-sm);background:${issues>0?'rgba(214,48,49,0.03)':'var(--green-ultra-pale)'};border:1px solid ${issues>0?'rgba(214,48,49,0.12)':'var(--green-pale)'}">
                    ...
                    <div style="font-size:0.75rem;color:var(--text-light);display:flex;justify-content:space-between">
                      ...
                      <span>${nextTask ? `Next: ${nextTask.name.substring(0,35)}...` : '✅ All tasks done'}</span>
                    </div>
                    <div style="font-size:0.78rem;color:var(--text-mid);margin-top:8px;font-style:italic">"${gh.notes}"</div>
                  </div>`;
              }).join('')}
            </div>
          </div>
        ...
        ${AFV.greenhouses.map(gh => {
          ...
          return `
            <div class="agro-report" style="margin-bottom:20px">
              ...
              <div style="font-size:0.85rem;color:var(--text-mid);margin-bottom:14px;font-style:italic;border-left:3px solid #9b59b6;padding-left:10px">"${gh.notes}"</div>
            ...
            <div class="card" style="margin-bottom:20px;border:1px solid #e8d5e8">
              ...
              <div class="scroll-x">
                <table>
                  <thead>
                    <tr>
                      <th>#</th><th>Task</th><th>Priority</th><th>Status</th><th>Completed By</th><th>Date</th><th>Agronomist Comment</th>
                    </tr>
                  </thead>
                  <tbody>
                    ${gh.tasks.map((task, idx) => {
                      ...
                      return `
                        <tr style="background:${task.completed?'transparent':'rgba(225,112,85,0.03)'}">
                          ...
                          <td>${task.completed ? '<span class="badge badge-green">✅ Done</span>' : !prevDone ? '<span class="badge badge-orange">🔒 Blocked</span>' : '<span class="badge badge-red">⏳ Pending</span>'}</td>
                          <td>${worker ? worker.name : task.completed ? '—' : 'Unassigned'}</td>
                          <td style="font-family:'JetBrains Mono',monospace;font-size:0.75rem">${task.completedAt ? task.completedAt.toLocaleDateString('en-KE') : '—'}</td>
                          <td>
                            ${!task.completed && !prevDone ? `<span style="font-size:0.75rem;color:var(--orange-warn)">Blocked: task ${idx} pending</span>` : 
                              !task.completed ? `<input placeholder="Add note..." style="font-size:0.75rem;padding:4px 8px;border:1px solid var(--green-pale);border-radius:4px;width:160px" onchange="AgronomistDashboard.addTaskNote(${gh.id},'${task.id}',this.value)">` : '✅'}
                          </td>
                        </tr>`;
                    }).join('')}
                  </tbody>
  ...
  addTaskNote(ghId, taskId, note) {
    console.log(`Agronomist note for GH${ghId} task ${taskId}: ${note}`);
